<template>
	<view class="custom_popup_radio" :class="{ popup_show: showPopup }">
		...
		<view class="popup_content">
			...
			<scroll-view scroll-y="true" class="schoolList" @scrolltolower="scroll">
				<radio-group v-if="range.length>0" @change="changeItem">
					<label v-for="(item,index) in range" :key="index" class="item">
						<text>{{item[rangeKey]}}</text>
						<radio color="#007AFF" :value="item.value+''" :checked="indexValue == item.value"/>
					</label>
				</radio-group>
